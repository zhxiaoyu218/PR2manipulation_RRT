...
public:
  ...
  int add(Configuration config,vector<RRTNode> &_nodes,int ID)
  {
   ...
   int NearestNode(Configuration Config, vector<RRTNode> nodes)
	{
		float dist = 100000.0;
		int index = -1;
		...
		{
        	if (distance(nodes[i].getConfig() , Config) < dist)
			{
					if (dist <= 0.01)
					{
						index = i;
					...
					{
						dist =distance(nodes[i].getConfig() , Config);
						index = i;
					}
				}
  ...
  {
        	env->GetRobot("PR2")->SetActiveDOFValues(Config);
